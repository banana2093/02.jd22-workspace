...
int broAge = 17;
int myAge = 18;
System.out.println("내가 알던 형은 형인가?"+(broAge>myAge));
System.out.println("내가 알던 형은 형인가?"+(broAge>myAge?"응,형맞아":"아니,동생이었어헐ㄹㄹ;;;;;"));
...
int age = 20;
// 논리곱
System.out.println("\n한 사람의 성별과 나이가 각각 남자이고 20세 이상이면 회원자격을 준다. 이 사람은 회원인가?"+(man=="남자"&&age>=20));
// 논리합
System.out.println("\n한 사람의 성별과 나이가 각각 남자이고 20세 이상이면 회원자격을 준다. 이 사람은 회원인가?"+(man=="남자"||age>=20));
...
// repeat(반복횟수)
var nayana = "나야나\n";
// var 로 선언하면 왼쪽의 데이터 형에 따라
// 자동으로 형을 정해주는 선언방벙(Java10부터 도입)
// 자바에서 주로 사용되지는 않으니 참고로 알아두자ㅏㅏㅏㅏㅏㅏㅏㅏ
System.out.println("나야나 열번반복하기:\n"+
nayana.repeat(10));
...
System.out.println(familyName.concat(Name2)); // "공룡"
...
// 4. Math.abs(x) - 절대값
System.out.println("나는 1000원이 있는데 10000원을 썼다. 순수차액은 얼마?"+Math.abs(1000-10000)+"원");
// 5. Math.random() - 0.0(포함)과 1.0(제외) 사이의 난수
// ceil() 올림, floor() 내림, round() 반올림
System.out.println("1~5사이의 난수는?"+Math.ceil(Math.random()*5));    
